...
_D_Linked_List_Node* new_D_Linked_List_Node(void* data) {
...
void* free_D_Linked_List_Node(_D_Linked_List_Node* self) {
	if (self) free(self);
	return NULL;
}
...
D_Linked_List* free_D_LL(D_Linked_List* list) {
	if (list) {
...
unsigned long D_LL_get_size(ll* list) {
	return list->_size;
}

void D_LL_append(D_Linked_List* list, void* data) {
	_D_Linked_List_Node* node = new_node(data);
...
void* ll_get(D_Linked_List* list, unsigned int index) {
...
void* ll_set(D_Linked_List* list, unsigned int index, void* data) {
	if (index >= list->_size) return NULL;
...
// Returns the data to be overwritten so client can easily free it.
void* ll_remove(D_Linked_List* list, unsigned int index) {
	if (index >= list->_size) return NULL;
...
// Returns the data to be overwritten so client can easily free it.
void* ll_remove_data(D_Linked_List* list, void* data) {
...
bool ll_has(D_Linked_List* list, void* data) {
	_D_Linked_List_Node* head = list->head;
...
void* ll_peak_head(D_Linked_List* list) {
	if (list->_size == 0) return NULL;
	
	return list->head->data;
}
	
void* ll_pop_head(D_Linked_List* list) {
	if (list->_size == 0) return NULL;
...
void* ll_peak_tail(D_Linked_List* list) {
	if (list->_size == 0) return NULL;
	
	return list->tail->data;
}

void* ll_pop_tail(D_Linked_List* list) {
	if (list->_size == 0) return NULL;
	
	void* ret = list->tail->data;
	ll_remove(list, list->_size-1);
	return ret;
}

// Mostly for debug
void ll_print(D_Linked_List* list) {
	if (list == NULL) return;
